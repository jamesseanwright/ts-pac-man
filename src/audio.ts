...
const mapToPromises = <TItem, TResolve>(
  items: TItem[],
  cb: PromiseMapCallback<TItem, TResolve>,
) => Promise.all(items.map(cb));

export const audioPlayerCreator = (fetch: typeof window.fetch) => async (
  ...
  const audioBuffers = await mapToPromises(arrayBuffers, ab =>
